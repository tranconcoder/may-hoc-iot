...
let frameRateSelect, qualitySelect, resolutionSelect, fpsModeSelect, fpsLimitContainer;
let sourceTypeSelect; // Thêm biến cho việc chọn nguồn ghi hình

// WebSocket connection
let socket = null;
let wsUrl = 'ws://172.28.31.150:3000?source=esp32cam_security_gate_send_img';
...
window.addEventListener('load', () => {
    ...
    // Đăng ký sự kiện để theo dõi khi tab bị ẩn/hiện
...
async function startCapture() {
    try {
        ...
        try {
            ...
            addLog(`Đã chọn nguồn ghi hình: ${surfaceType} (${settings.width}x${settings.height})`);
...
function scheduleReconnect() {
    ...
    addLog(`Lên lịch kết nối lại lần thứ ${reconnectAttempts} sau ${Math.round(timeout/1000)} giây...`);
...
function enableContinuousHighPerformanceMode() {
    ...
    function captureAndSendFrame() {
        if (!isRecording || !socket || socket.readyState !== WebSocket.OPEN || sendingFrame) return;
    ...
    if (!captureWorker) {
        try {
            ...
            const targetFps = fpsModeSelect.value === 'unlimited' 
            ...
            addLog(`Đã kích hoạt worker xử lý nền với tốc độ FPS cao nhất (${targetFps} FPS)`);
        } catch (error) {
...
window.addEventListener('DOMContentLoaded', () => {
    // Do nothing, init is handled in window.onload
});
